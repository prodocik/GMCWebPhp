  <div class="container-fluid">
            <div class=" promotext col-lg-12 col-md-12 col-xs-12">
                <h1>2018 Heartland Mallard M185</h1>

    </div>
        </div>
       </header>
<div class="content">
        <div class="product-item">
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        <h1>2018 Heartland Mallard M185</h1>
                        <div class="slider">
                            <div class="slider-for">
                                <div>
                                    <div class="price"><small>Regular Daily Rate</small><span><big><?php echo $mallard_1_RegDaily; ?></big></span></div><img src="images/more/mallard_1/1.jpg" alt="Heartland Mallard 3/4"></div>
                                <div>
                                    <div class="price"><small>Regular Daily Rate</small><span><big><?php echo $mallard_1_RegDaily; ?></big></span></div><img src="images/more/mallard_1/2.jpg" alt="Heartland Mallard front slideout"></div>
                                <div>
                                    <div class="price"><small>Regular Daily Rate</small><span><big><?php echo $mallard_1_RegDaily; ?></big></span></div><img src="images/more/mallard_1/3.jpg" alt="Heartland Mallard left side"></div>
                                <div>
                                    <div class="price"><small>Regular Daily Rate</small><span><big><?php echo $mallard_1_RegDaily; ?></big></span></div><img src="images/more/mallard_1/4.jpg" alt="Heartland Mallard right side slideout"></div>
                                <div>
                                    <div class="price"><small>Regular Daily Rate</small><span><big><?php echo $mallard_1_RegDaily; ?></big></span></div><img src="images/more/mallard_1/5.jpg" alt="Heartland Mallard kitchen"></div>
                                
                            </div>
                            <div class="slider-nav">
                                <div><img src="images/more/mallard_1/1_small.jpg" alt="Heartland Mallard 3/4 preview"></div>
                                <div><img src="images/more/mallard_1/2_small.jpg" alt="Heartland Mallard front slideout preview"></div>
                                <div><img src="images/more/mallard_1/3_small.jpg" alt="Heartland Mallard left side preview"></div>
                                <div><img src="images/more/mallard_1/4_small.jpg" alt="Heartland Mallard right side slideout preview"></div>
                                <div><img src="images/more/mallard_1/5_small.jpg" alt="Heartland Mallard kitchen preview"></div>

                            </div>
                        </div>
                        <div class="desc">
                            <p>Mallard, Heartland RV: Mallard is a full featured travel trailer in a smaller, lighter package. You will appreciate the openness of the Mallard layouts featuring an arched ceiling measuring in at nearly seven feet. Mallard gives you room for the whole family to relax, dine or entertain. Oversized windows let in natural light and spectacular views of the great outdoors. Mallard provides kitchens with efficient wide-open prep areas, as well as large base cabinets and huge overhead storage for that extended vacation. Mallard offers models to fit your specific needs whether your a couple just getting away or a family of six with friends choose Mallard and migrate to your favorite get away.</p>
                            <div class="smoking"><p>Smoking is not allowed <img src="images/no-smoking.svg" alt="no_smoking" width="20px"></p> </div>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-12 col-sm-12">
                        <div class="attr">
                          <ul>
                              <li>Sleeping Capacity: <span>3-4 people</span></li>
                              <li>Lenght: <span>21'11"</span></li>
                              <li>Bed Size: <span>Queen</span></li>
                              <li>Air conditioner: <span>YES</span></li>
                              <li>Refrigirator: <span>YES</span></li>
                              <li>Fresh Water Cap.: <span>52 gal.</span></li>
                              <li>Grey Water Capacity: <span>42 gal.</span></li>
                              <li>Black Water Capacity: <span>42 gal.</span></li>
                          </ul>
                          <ul>
                              <li>Converter: <span>55 AMP / 12 V</span></li>
                              <li>Electric: <span>30 AMP</span></li>
                              <li>Fuel Efficiency: <span>15 MPG</span></li>
                              <li>LP Capacity: <span>(2) 20 lbs</span></li>
                              <li>Generator: <span>Not available</span></li>
                          </ul>
                            <div class="clearfix"></div>
                        </div>
                    </div>

                </div>
                <div class="row">
                    <div class="col-md-4 col-sm-4">
                        <div class="rates">
                            <span class="h">REGULAR RATES</span>
                            <span class="name">Daily</span>
                            <span class="price"><?php echo $mallard_1_RegDaily; ?></span>
                            <span class="name">Weekly</span>
                            <span class="price"><?php echo $mallard_1_RegWeekly; ?></span>
                            <span class="name">Monthly</span>
                            <span class="price"><?php echo $mallard_1_RegMonthly; ?></span>
                        </div>
                    </div>
                    <div class="col-md-4 col-sm-4">
                        <div class="rates">
                            <span class="h">PEAK RATES*</span>
                            <span class="name">Daily</span>
                            <span class="price"><?php echo $mallard_1_HighDaily; ?></span>
                            <span class="name">Weekly</span>
                            <span class="price"><?php echo $mallard_1_HighWeekly; ?></span>
                            <span class="name">Monthly</span>
                            <span class="price"><?php echo $mallard_1_HighMonthly; ?></span>
                        </div>
                    </div>
                    <div class="col-md-4 col-sm-4">
                        <div class="rates">
                            <span class="h">WINTER RATES*</span>
                            <span class="name">Daily</span>
                            <span class="price"><?php echo $mallard_1_LowDaily; ?></span>
                            <span class="name">Weekly</span>
                            <span class="price"><?php echo $mallard_1_LowWeekly; ?></span>
                            <span class="name">Monthly</span>
                            <span class="price"><?php echo $mallard_1_LowMonthly; ?></span>
                        </div>
                    </div>
                    <div class="row infotext">
                        <p>*Learn more about season rates<a href="pricing">here</a></p>
                    </div>
                </div>
                <div class="row">
                    <div class="navigation">
                        <div class="col-lg-3 col-md-2 col-sm-2 col-xs-1">
                            <a href="mallard_2.html" class="arr-left"></a>
                        </div>
                        <div class="col-lg-6 col-md-8 col-sm-8 col-xs-10">
                            <div id="btn_book"><a href="mallard_1_booking.html">BOOK NOW</a></div>
                        </div>
                        <div class="col-lg-3 col-md-2 col-sm-2 col-xs-1">
                            <a href="mallard_2.html" class="arr-right"></a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
